...
% in db
t_hand = 20*log10(d.t_0);

a_cl_hand = 20*log10(720);
...
a_cl_3db_err = calc_err_pct(d.f_cl, a_cl_3db_spice);
...
s = sprintf('Closed Loop 3dB:\nHand: %0.1fGHz\nSpice: %0.1fGHz\nError: %0.1f%%', d.f_cl/1e9, a_cl_3db_spice/1e9, a_cl_3db_err);
text(1e8, 30, s)

s = sprintf('Gain Peaking = %0.2fdB', max(vo_mag) - a_cl_spice)
text(1e9, 60, s)
