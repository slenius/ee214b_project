...
% (1) Lookup VGS with known source voltage
% (2) Lookup VGS with unknown source voltage, e.g. when the source of the
% transistor is the tail node of a differential pair
%
% In usage mode (1), the input to the function is gm/ID, VDS and VSB.
% For example:
%
% VGS = lookupVGS(data, GM_ID, 10, 'VDS', 0.6, 'VSB', 0.1, 'L', 0.1)
%
% When VS, VDS or L are not specified, their default values are assumed:
%
% VSB = 0;
...
%
% The default interpolation method for is "pchip". It can be set to a 
% different method by e.g. by passing 'METHOD', 'linear' to the function.
%
